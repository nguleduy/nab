...
@Aspect
@Component
public class CustomerBehaviorAspect {
  ...
  public CustomerBehaviorAspect(Source source) {
    this.source = source;
  }

  @Before("execution(* com.example.joseph.productservice.controller.ProductController.getProductDetail(..))")
  @Async
  public void logUserViewProduct(JoinPoint joinPoint) {
    Object[] methodArguments = joinPoint.getArgs();
    Long productId = (Long) methodArguments[0];
    String username = (String) methodArguments[1];
    CustomerBehavior customerBehavior = new CustomerBehavior(username, LocalDateTime.now(), productId);
    Message<CustomerBehavior> message = MessageBuilder.withPayload(customerBehavior).build();
    source.output().send(message);
  }

  @Before("execution(* com.example.joseph.productservice.controller.ProductController.getAllProducts(..))")
  @Async
  public void logUserFilterAndSort(JoinPoint joinPoint) {
    ...
    if ((sortBy != null && !sortBy.isEmpty()) || (!filterBy.isEmpty())) {
      ...
      CustomerBehavior customerBehavior = new CustomerBehavior(username,
